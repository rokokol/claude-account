#!/usr/bin/env bash
# Distro tests for claude-account: run install.sh for real, as root, inside a container
# of an actual distribution — the one thing tests/run.sh cannot do. Here the real
# package manager provides jq and pgrep, by running the very commands the preflight
# printed when it refused. claude itself is deliberately not checked anywhere — the
# tool manages its profiles whether or not the binary is here yet.
#
...
# because the answer was planted. Arch alone needs its sync database refreshed: the
# printed pacman -S cannot work against an empty one, and no reader of a refusal is
# ever told to type the refresh
declare -A BOOTSTRAP=(
  [debian]=':'
  [ubuntu]=':'
...
smoke() { # runs inside the container after a successful install
  local prefix="$1"
  "$prefix/bin/claude-account" --version | grep -qxF "claude-account $(cat VERSION)"
  "$prefix/bin/claude-account" --help >/dev/null
  # The whole behaviour suite against the installed copy, pgrep stubbed by the suite
  CLAUDE_ACCOUNT="$prefix/bin/claude-account" ./tests/run.sh >/dev/null
}
...
prefix=/usr/local
bin_path="$prefix/bin/claude-account"
share_dir="$prefix/share/claude-account"

say "a relative PREFIX is rejected with exit 2, the usage-error code"
rc=0
PREFIX=usr ./install.sh "${INSTALL_FLAGS[@]}" >/dev/null 2>&1 || rc=$?
((rc == 2)) || die "a relative PREFIX exited $rc, not the usage-error code 2"

say "an unknown flag is rejected with exit 2, the usage-error code"
rc=0
./install.sh --no-such-flag "${INSTALL_FLAGS[@]}" >/dev/null 2>&1 || rc=$?
...
if ((rc != 0)); then
  ...
  commands=$(printf '%s\n' "$out" | sed -n 's/^  \$ //p')
  if [[ -z "$commands" ]]; then
    echo "::notice title=claude-account distro test::SKIP on $distro — guidance is manual-only"
